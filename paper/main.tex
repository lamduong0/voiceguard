...
four pretrained detectors, evaluated at a matched false-alarm rate with out-of-fold fusion
...
high-intent call, beats linear (weighted-sum) fusion in two of the detector$\times$dataset
settings tested (8/8 random seeds, non-overlapping CIs), but the gain is modest
($\approx$0.12--0.14 TPR) and detector-specific---two other detectors show none, and the two
tie when the detector is near-perfect or broken; and (3) detector
...
ASVspoof, detector m1, 600 clips/class/source, 8 seeds (mean $\pm$ std):
\begin{center}\small
\begin{tabular}{lccc}
\toprule
method & AUC & TPR@10\%FAR vs gen.\_scam & TPR@10\%FAR vs all-neg \\
\midrule
acoustic\_only & $0.770\pm0.006$ & $0.679\pm0.031$ & $0.406\pm0.026$ \\
intent\_only & $0.894\pm0.009$ & $0.000\pm0.000$ & $0.000\pm0.000$ \\
parallel & $0.933\pm0.010$ & $0.484\pm0.067$ & $0.708\pm0.064$ \\
bayesian & $\mathbf{0.950\pm0.005}$ & $0.606\pm0.053$ & $\mathbf{0.804\pm0.035}$ \\
\bottomrule
\end{tabular}
\end{center}
Intent alone is useless against \textsc{genuine\_scam} (0.000); acoustic alone is weak
against the broad negative set (0.406). Only fusion is strong on both axes
(Figure~\ref{fig:roc}).
...
bayesian $-$ parallel, TPR@10\%FAR vs \textsc{genuine\_scam}, 8 seeds (600 clips for m1/m2; 300 for m3/m4):
\begin{center}\small
\begin{tabular}{llcc}
\toprule
setting & acoustic regime & bayesian $-$ parallel & seeds won \\
\midrule
m1 $\times$ ASVspoof & informative, imperfect & $+0.122$ & 8/8 \\
m2 $\times$ In-the-Wild & informative, imperfect & $+0.135$ & 8/8 \\
m2 $\times$ ASVspoof & near-perfect & $-0.010$ & tie \\
m4 $\times$ ASVspoof & near-perfect (clean) & $-0.018$ & tie \\
m4 $\times$ In-the-Wild & informative (clean) & $-0.031$ & no win \\
m1 $\times$ In-the-Wild & broken & $+0.033$ & both $\approx$0 \\
\bottomrule
\end{tabular}
\end{center}
Where it appears (m1$\times$ASVspoof, m2$\times$In-the-Wild) the multiplicative veto wins
across all 8 seeds and the bootstrap CIs do not overlap (m1$\times$ASVspoof at 600 clips:
bayesian $[0.604,0.707]$ vs parallel $[0.422,0.533]$). But the scaled-up run (four
detectors, 600 clips) shows the effect is \emph{modest and not universal}. (i) With more
data the m1$\times$ASVspoof gain shrank from $+0.251$ (300 clips) to $+0.122$ (600), so the
small-sample estimate was inflated. (ii) Only two of four detectors show a clean win: a
third (mo-thecreator) collapses both classes to P(synthetic)\,$\approx$\,0 under additive
noise rather than degrading gracefully (negative gain at every SNR), and a fourth (Hemgg)
that \emph{does} degrade gracefully still shows no win in its informative band. The veto
helps in specific detector$\times$regime combinations with a clear mechanism, but is not a
general property of multiplicative fusion, and AUC level alone does not predict it.
...
is LLM-generated and LLM-scored, hence somewhat idealized. We evaluate four detectors, two
datasets, and one language, at modest scale (multi-seed reporting), and one degradation type
(additive noise); the veto's benefit is detector-specific (a clean win in two of four
detectors) and is not predicted by graceful degradation or AUC level alone (\S\ref{sec:c2}).
We study defense only,
...
\section{Conclusion}
Fusing intent with an acoustic deepfake detector can help---conditionally and modestly. A
multiplicative acoustic veto beat linear fusion in two of four detectors
($\approx$0.12--0.14 TPR, seed-robust, CIs separate) with a clear mechanism, and did nothing
in the other two, so it is a tool to evaluate per detector rather than a universal rule. The
dominant lever is acoustic-detector generalization: when it fails, no fusion recovers it.
